...
export const addReferalUser = async (req, res, next) => {
    try {
        ...
        if (user) {
            await sendMail(user.email, user.username, user.ownSponserId, transactionPassword, password);
...
export const viewAllTransactions = async (req, res) => {
    try {
        ...
        if (userData) {
            ...
            const allTransactions = [...referalHistory, ...level1ROIHistory, ...level2ROIHistory, ...level3ROIHistory, ...dailyROIHistory, ...walletWithdrawHistory, ...capitalWithdrawHistory];
...
export const viewAllPackage = async (req, res, next) => {
    ...
    try {
        ...
        if (userData) {
            ...
            res.status(200).json({ PackageData, sts: '01', msg: 'Successfully Updated' });
        } else {
            next(errorHandler('User Not Found'));
        }
    } catch (error) {
...
export const viewUserPackageDetails = async (req, res, next) => {
    ...
    try {
        ...
        if (userData) {
            ...
            console.log(PackageData);
            if (PackageData) {
                res.status(200).json({ PackageData, sts: '01', msg: 'Successfully Found Package data' });
            }
        } else {
            next(errorHandler('User Not Found'));
        }
