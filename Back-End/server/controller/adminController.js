...
export const adminLogin = async (req, res, next) => {
    ...
    try {
        ...
        if (validAdmin.isSuperAdmin) {
            ...
            const token = jwt.sign({ userId: validAdmin._id }, process.env.JWT_SECRET, {
                expiresIn: '365d',
            });
...
export const viewAllUsers = async (req, res, next) => {
    ...
    try {
        ...
        const userData = await User.find({ isSuperAdmin: { $ne: true } }).select('username email phone userStatus');
        if (user) {
